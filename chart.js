...
let letterScale
  //for(var l=2; l<data2.length;l++){
   letterScale = d3.scaleBand()
     ...
     .domain(["Other Offenses", "Motor Vehicle Theft?", "Rape","Drug Violantion","Suicide","Homocide","Liquor Laws", "Suspecious","Vehicle Impounded","Juvenile Offenses","Vehicle Misplaced", "Family Offense", "Fire Report", "Embezzlement", "Vandalism","Traffic Collision","Prostitution" ,"Civil Sidewalks","Courtesy Report", "Arson","Case Closure","Sex Offense","Weapons Carrying Etc","Forgery And Counterfeiting", "Stolen Property", "Weapons Offense", "Other", "Miscellaneous Investigation","Traffic Violation Arrest", "Disorderly Conduct", "Suspicious Occ","Drug Offense", "Offences Against The Family And Children", "Missing Person", "Fraud", "Recovered Vehicle", "Robbery", "Motor Vehicle Theft", "Lost Property","Warrant", "Burglary", "Malicious Mischief","Non-Criminal", "Assault","Other Miscellaneous", "Larceny Theft","Gambling", "Motor Vehicle Theft?", "Rape","Drug Violation", "Suicide","Homocide", "Liquor Laws", "Suspicious", "Vehicle Impounded"])
...
//////////////////////////////////////
    ...
    if (d.key==="Assault" || d.key=== "Arson" || d.key==="Burglary"|| d.key==="Family Offense" || d.key==="Homocide" || d.key==="Rape" || d.key==="Robbery" || d.key==="Sex Offense" || d.key==="Warrant" || d.key==="Weapons Offense" ) { //Red
      ...
      return "blue"
    }else if(d.key==="Case Closure" || d.key==="Civil Sidewalks" || d.key==="Courtesy Report" || d.key==="Emblezzement" || d.key==="Fraud" || d.key==="Liquor Laws" || d.key==="Lost Property" || d.key==="Miscellaneous Investigation" || d.key==="Missing Person" || d.key==="Other" || d.key==="Other Miscellaneous" || d.key==="Other Offenses" || d.key==="Recovered Vehicle" || d.key==="Suicide" || d.key==="Traffic Collision" || d.key==="Traffic Violation Arrest" || d.key==="Vehicle Impounded") {  //green
      return "green";
